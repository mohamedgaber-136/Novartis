...
export const Dashboard = () => {
    ...
    const {getData,EventRefrence,roleCondition} = useContext(FireBaseContext)
    const [data,setData]=useState([])
    useEffect(()=>{
      getData(EventRefrence,setData)
  },[]) 
        return  <div className="d-flex flex-column gap-4 align-items-start ">
            {/* <Notification/> */}
    ...
    {
        WeeksInfo.map((item,ind)=><WeekEeventsCard key={ind} num={data.length} calen={item.calen} info={item.times} color={item.color} />
        )
    }
        </div>
        <div className="w-100">
